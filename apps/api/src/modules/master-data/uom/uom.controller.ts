...
export class UomController {
	...
	@ApiBearerAuth('access-token')
	@ApiOperation({
		summary: 'List all units of measure (including inactive)',
	})
	@ApiResponse({
		status: 200,
	...
	@Roles('platform_admin')
	@ApiBearerAuth('access-token')
	@ApiOperation({ summary: 'Get a single unit of measure by ID' })
	...
	@Roles('platform_admin')
	@ApiBearerAuth('access-token')
	@ApiOperation({ summary: 'Update unit of measure name / code' })
	@ApiParam({ name: 'id', type: 'string', format: 'uuid' })
	...
	@ApiBearerAuth('access-token')
	@ApiOperation({
		summary: 'Soft-disable a unit of measure (BR-70)',
	})
	@ApiParam({ name: 'id', type: 'string', format: 'uuid' })
	...
	@ApiBearerAuth('access-token')
	@ApiOperation({
		summary: 'Re-enable a disabled unit of measure',
	})
	@ApiParam({ name: 'id', type: 'string', format: 'uuid' })
	...
	@ApiBearerAuth('access-token')
	@ApiOperation({
		summary: 'List active units of measure (for dropdowns)',
	})
	@ApiResponse({
		status: 200,
