...
export class RfqController {
	...
	@ApiOperation({ summary: 'Create a Draft RFQ (US-62)' })
	...
	@ApiOperation({
		summary: 'List RFQs visible to the caller (RLS by role)',
		description:
	...
	@HttpCode(HttpStatus.CREATED)
	@ApiOperation({
		summary: 'Add an item to a Draft RFQ (auto-merges duplicates, BR-387)',
	})
	@ApiParam({ name: 'id', type: 'string', format: 'uuid' })
	...
	@UseGuards(JwtAuthGuard, RbacGuard)
	@Roles(...BUYER_ROLES)
	@ApiOperation({ summary: 'Update an item in a Draft RFQ' })
	@ApiParam({ name: 'id', type: 'string', format: 'uuid' })
	...
	@UseGuards(JwtAuthGuard, RbacGuard)
	@Roles(...BUYER_ROLES)
	@ApiOperation({ summary: 'List quotations received for an RFQ' })
