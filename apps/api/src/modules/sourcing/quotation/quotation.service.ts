...
export class QuotationService {
	...
	async negotiate(
		...
		const round = await getDatabase().transaction(async (tx) => {
			// Turn-based enforcement: latest round's role cannot equal caller's role
	...
	async acceptLatestRound(
		...
		const result = await getDatabase().transaction(async (tx) => {
			...
			// You cannot accept your own proposal (UC31)
			...
			// so the Buyer can still execute selectQuotation(); only price/delivery
			// are mutated here.
			const updatedQuotation = await this.quotationRepo.updateQuotation(
				quotationId,
				{
					unitPrice: target.proposedPrice,
					// proposed_delivery_days is a negotiation-only field; we do NOT
					// overwrite estimated_delivery_date here per the docs.
				},
				tx,
			);
	...
	async selectQuotation(
		...
		// before we acquire any row locks (per the guide's note).
