...
- [ ] `POST /quotations/:id/select`: full atomic transaction per pseudocode above; complete in < 2s; snapshot all final_* fields into PO; reject competing quotations; cancel non-selected Child RFQs; audit log inside transaction
